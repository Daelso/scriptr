...
### Publish (EPUB export)
EPUB generation lives in [lib/publish/](lib/publish/) and is exposed via `/api/stories/[slug]/export/epub`. We ship both EPUB3 (KDP/modern readers) and EPUB2 (Smashwords-only). Gotchas: `epub-gen-memory` takes cover options as a **positional vararg**, and disk paths for `cover` MUST be `file://` URLs (use `pathToFileURL`) — bare absolute paths silently produce 0-byte covers. `@likecoin/epubcheck-ts` (>= 0.6.2) validation is wired up and functional: `validateEpub` in [lib/publish/epub.ts](lib/publish/epub.ts) loads it via dynamic `import()`, and both it and its `libxml2-wasm` WASM dep are in `serverExternalPackages` ([next.config.ts](next.config.ts)) so webpack doesn't bundle them. Earlier (<= 0.6.0) the package did a top-level `require('libxml2-wasm')` of an ESM/top-level-await module, which crashed under Next.js 16's webpack bundle and left validation silently non-functional; don't reintroduce a `require()` of it.

EPUB export also appends an optional "A note from the author" content entry per pen-name. Resolution + HTML build live in [lib/publish/author-note.ts](lib/publish/author-note.ts); profile data is stored in `data/config.json` under `penNameProfiles`. The note's QR `<img>` is post-processed in `buildEpubBytes` (data: URL → temp PNG file → `file://` URL) so `epub-gen-memory`'s image-extraction pass works correctly. Client components import the sanitize allowlist from [lib/publish/author-note-shared.ts](lib/publish/author-note-shared.ts) (a dependency-free split) to keep `qrcode` out of client bundles.
