...
5. **`walk.ts`** — for each `NavEntry` in nav order:
   ...
   - **With anchor:** find element with that `id`. Take everything from that element up to (but not including) the next consecutive nav entry's anchor *in the same file*. If the next nav entry points to a different file, take everything to end of `<body>`. Drives Pattern Z (one XHTML, multiple chapters).
...
5. If `importCover && sessionId`: pull bytes from cover-cache. Behavior by mime: JPEG → write as-is via `writeCoverJpeg`; PNG/WebP → transcode to JPEG via `sharp` (existing pattern), then write; `application/octet-stream` (unknown magic bytes) → skip cover write, log warning, continue. Cover-cache entry deleted in a `finally` so a downstream chapter-write failure doesn't strand it until TTL.
...
**Why not reuse [components/import/ChapterEditList.tsx](../../../components/import/ChapterEditList.tsx):** the existing list is typed against NovelAI's `ProposedChapter` + `SplitSource` and exposes delete + merge actions. EPUB needs an include/exclude checkbox, a `skipReason` tooltip, and `nav` / `spine` / `skipped` badges instead. Genericizing the existing component to cover both shapes would add more conditional UI than a fresh ~60-line component costs. If a future importer needs the same checkbox-and-badge pattern, the right time to extract is when there are two callers, not one.
...
- Nav entry's `href` points to a file that's not in the zip/manifest → entry skipped silently with a debug log (don't fail the whole import for one missing chapter).
...
**Watch:** Vitest's `test:watch` doesn't re-run npm scripts on file change, so contributors editing `scripts/build-epub-fixtures.ts` need to manually re-run `npm run pretest` (or whatever the script is named) to regenerate fixtures before watch reruns pick them up. Document this near the script with a one-line comment.
