...
3. **One scheduled network call: the update check, and only when you've enabled it.** If "Check for updates on launch" is on in Settings (default-on after first-run onboarding completes; off entirely until then), the desktop app contacts GitHub's release feed exactly once per launch. Nothing on a timer, nothing per-keystroke, nothing in the renderer. There are no analytics, version pings, or status pollers. Every other request is on explicit user action — and you can disable the update check at any time.
